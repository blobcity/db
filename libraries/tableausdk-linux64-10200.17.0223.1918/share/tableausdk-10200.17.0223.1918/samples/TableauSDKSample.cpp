...
{
    std::cerr
        << "usage: tableauSDKSample    [-h] [-b] [-s] [-p] [-o] [-f FILENAME]" << std::endl
...
void
PopulateExtract(
    const std::shared_ptr<Extract> extractPtr,
    bool useSpatial
)
{
    try {
...
main( int argc, char* argv[] )
{
    ...
    if ( options.count( "publish" ) > 0 ) {
        ...
        //  Publish the Extract
        PublishExtract( options[ "filename" ], options[ "project-name" ], options[ "datasource-name" ], options.count( "overwrite" ) > 0,
