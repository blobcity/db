 ...
 * @author sanketsarang
 */
public class UpdateEvent implements HookEvent {
    private final JSONObject jsonObject;

    public UpdateEvent(final String ds, final String collection, final JSONObject record) {
    ...
    public UpdateEvent(final String ds, final String collection, List<JSONObject> records) {
        jsonObject = new JSONObject();
        jsonObject.put(Protocol.EVENT.key(), Protocol.UPDATE_EVENT.key());
        jsonObject.put(Protocol.DS.key(), ds);
        jsonObject.put(Protocol.COLLECTION.key(), collection);
