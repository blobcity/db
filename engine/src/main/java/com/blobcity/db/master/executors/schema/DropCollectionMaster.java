...
/**
 * Master that handles drop-collection requests
 *
 * @author sanketsarang
 */
public class DropCollectionMaster extends ExecuteAllNodesCommitMaster implements MasterExecutable {

    final String archiveCode;

    public DropCollectionMaster(Query query) throws OperationException {
        super(query);

        if(query.getQueryType() != QueryType.DROP_COLLECTION && query.getQueryType() != QueryType.DROP_TABLE) {
            throw new OperationException(ErrorCode.INTERNAL_OPERATION_ERROR, "Incorrect query passed to drop-collection master");
        }
