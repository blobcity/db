 ...
 * @author sanketsarang
 */
public class DropCollectionProcessor extends AbstractCommitProcessor implements Processor {

    public DropCollectionProcessor(Query query) {
        super(query);
    }
    ...
    public void softCommit() {
        ...
        while(attempts++ < 3) {
            if (super.transientStore.acquireCollectionPermit(ds, collection)) {
                try {
                    collectionManager.dropTable(ds, collection);
    ...
    public void rollback() {
        ...
        try {
            if(doesRollbackNeedAction()) {
                super.getBean(BSqlCollectionManager.class).undoDropCollection(super.query.getString(QueryParams.ARCHIVE_CODE));
            }
