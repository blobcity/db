...
/**
 * Used to get files by processing files names that match the reference value in the desired operation. This class satisfies all files that satisfy not-equals
 * operator with reference value.
 *
 * @author sanketsarang
 * @param <T>
 */
public class NEQFilenameFilter<T extends Comparable<T>> implements OperatorFileFilter {

    private final TypeConverter<T> typeConverter;
    private final T referenceValue;

    public NEQFilenameFilter(TypeConverter<T> typeConverter, T referenceValue) {
    ...
    public boolean accept(Object entry) throws IOException {
        try {
            ...
            LoggerFactory.getLogger(NEQFilenameFilter.class.getName()).error(null, ex);
