...
/**
 * Used to get files by processing files names that match the reference value in the desired operation. This class satisfies all files that satisfy NOT IN check
 * with reference values.
 *
 * This operator should be used for queries of the type SELECT * FROM table WHERE NOT col IN (val1, val2)
 *
 ...
 * @author sanketsarang
 */
public class NotInFilenameFilter<T> implements OperatorFileFilter {

    private final TypeConverter<T> typeConverter;
    private final Set<Object> searchSet;

    public NotInFilenameFilter(TypeConverter typeConverter, Set<Object> searchSet) {
    ...
    public Object getTypeConvertedReferenceValue() throws OperationException {
        throw new UnsupportedOperationException("Operation not supported. Invalid invocation case for getTypeConvertedReferenceValue() on NOT IN operator");
    }
}
